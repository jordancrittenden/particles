...
TracerBuffers create_tracer_buffers(wgpu::Device& device, const std::vector<glm::f32vec4>& loc) {
    ...
    float sep = 0.01f * _M;  // Make separation larger so tracers are visible
    for (auto& pos : loc) {
    ...
    std::cout << "Created " << loc.size() << " tracers with " << TRACER_LENGTH << " points each = " << tracerTrails.size() << " total vertices" << std::endl;
    if (!tracerTrails.empty()) {
        std::cout << "First tracer position: " << tracerTrails[0].x << ", " << tracerTrails[0].y << ", " << tracerTrails[0].z << std::endl;
        std::cout << "Last tracer position: " << tracerTrails.back().x << ", " << tracerTrails.back().y << ", " << tracerTrails.back().z << std::endl;
    }
